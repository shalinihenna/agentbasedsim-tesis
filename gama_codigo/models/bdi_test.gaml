...
species miner skills:[moving] control:simple_bdi {
	...
	//when the agent initializes, its first desire is to find gold
	init{
    ...
    perceive target: gold_mine where (each.quantity > 0) in: view_dist {	//detects gold_mines that are not empty (quantity > 0)
    	focus id: mine_at_location var:location;							//adds each detected gold mine to the beliefs
    	ask myself {
    		do remove_intention(find_gold, false); 							//false so that the 'find_gold' doesnt get removed from the desire_base, so that in the future the agent can still find gold
    	}
    }
    ...
    //plan to find_gold intention
    plan lets_wander intention: find_gold{
    	do wander;
    }
    
    //plan to has_gold intention
    	...
    	if( target = nil ){
    		...
    		if(target = location){
    			...
    			if current_mine.quantity > 0 {
    				do add_belief(has_gold); //loop hasta que se acabe el gold
